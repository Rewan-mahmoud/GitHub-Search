    ...
    <div>
           <section className="user-card">
            <div className="user-top">
                <img className="avatar" src={user?.data?.avatar_url} alt={user.name}/>
                <div className="user-info">
                    <div className="user-info-inner">
                        <h2 className="name">
                        {user?.data?.login}
                        </h2>
                        <a className="login" href={user?.data?.html_url} target="_blank" rel="noreferrer">
                            @{user?.data?.login}
                        </a>
                    </div>
                    <p className="joined"> Joined {user?.data?.created_at} </p>
                </div>
            </div>
            <div className="user-middle">
                <p className="bio">
                    {user?.data?.bio ? <p>{user?.data?.bio}</p> : "This profile has no bio"}
                </p>
                <div className="stats">
                    <div className="stat">
                        <h3 className="stat-title">Repos</h3>
                        <p className="stat-number">{user?.data?.public_repos}</p>
                    </div>
                    <div className="stat">
                        <h3 className="stat-title">Followers</h3>
                        <p className="stat-number">{user?.data?.followers}</p>
                    </div>
                    <div className="stat">
                        <h3 className="stat-title">Following</h3>
                        <p className="stat-number">{user?.data?.following}</p>
                    </div>
                </div>
            </div>
            <div className="user-bottom">
                <div className="links">
                  ...
                  {user?.data?.location? <p>{user.data.location}</p> : "Not available"} 
                    </div>
                    <div className="link-wrapper">
                        <svg height="20" width="20" xmlns="http://www.w3.org/2000/svg">
                            <g fill="currentColor">
                        ...
                        </svg>
                        {
                            !user?.data?.blog ?
                            <p>Not Available</p> :
                            <a href={user.data.blog} target="_blank" rel="noreferrer">{user.data.blog}</a>
                        }
                    </div>
                </div>
                <div className="links">
                    <div className="link-wrapper" >
                        ...
                        {user?.data?.twitter_username? <p>{user.data.twitter_username}</p> : "Not available"} 
                    </div>
                    <div className="link-wrapper">
                        <svg height="20" width="20" xmlns="http://www.w3.org/2000/svg">
                            <g fill="currentColor">
                        ...
                        {user?.data?.company? <p className="mt-3">{user.data.company}</p> : "Not available"} 
